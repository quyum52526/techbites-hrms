...
export default function PayPulsePayrollTable({ rows, departments, month }: { rows: PayrollRow[]; departments: string[]; month: string }) {
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    return (!query || row.name.toLowerCase().includes(query) || row.code.toLowerCase().includes(query)) &&
      (!department || row.department === department) && (!payModel || row.payModel === payModel);
  }), [rows, search, department, payModel]);

  function exportCsv() {
    const header = ["Employee Code", "Name", "Department", "Pay Model", "Logged Metrics", "Gross", "Deductions", "Net Payable", "Status"];
    const body = filtered.map((row) => [row.code, row.name, row.department, row.payModel, `${row.loggedValue} ${row.loggedUnit}`, row.gross, row.deductions, row.net, row.status]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">Department
          <select value={department} onChange={(event) => setDepartment(event.target.value)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900"><option value="">All Departments</option>{departments.map((item) => <option key={item}>{item}</option>)}</select>
        </label>
