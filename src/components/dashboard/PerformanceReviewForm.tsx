  ...
  return (
    <div className="space-y-2 min-w-44">
      {(["productivity", "qualityOfWork", "collaboration"] as const).map((metric) => (
        <label key={metric} className="flex items-center justify-between gap-2 text-[10px] text-slate-500">
          ...
          <span className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((score) => (
              <button key={score} type="button" title={`${score} out of 5`} onClick={() => setMetrics({ ...metrics, [metric]: score })}>
                <Star className={`w-3 h-3 ${score <= metrics[metric] ? "fill-amber-400 text-amber-400" : "text-slate-300"}`} />
              </button>
            ))}
          </span>
        </label>
      ))}
      <textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="Feedback" rows={2} className="w-full border border-slate-200 rounded-md p-1.5 text-[10px] outline-none focus:ring-1 focus:ring-indigo-500" />
      <button type="button" disabled={saving} onClick={save} className="w-full py-1.5 rounded-md bg-indigo-600 text-white text-[10px] font-semibold disabled:opacity-50">
